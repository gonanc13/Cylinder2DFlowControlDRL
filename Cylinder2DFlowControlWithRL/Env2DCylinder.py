...
class Env2DCylinder(Environment):
    ...
    def show_flow(self):
        ...
        plt.xlim([-self.geometry_params['front_distance'],
                  self.geometry_params['length'] - self.geometry_params['front_distance']])
        plt.ylim([-self.geometry_params['bottom_distance'],
    ...
    def sing_run_output(self):
        ...
        if (not os.path.exists("saved_models/" + name)):
            with open("saved_models/" + name, "w") as csv_file:
                ...
                spam_writer.writerow([self.simu_name, self.solver_step, self.history_parameters["drag"].get()[-1],
                                      self.history_parameters["lift"].get()[-1],
    ...
    def compute_reward(self):
        ...
        if self.reward_function == 'plain_drag':  # a bit dangerous, may be injecting some momentum
            ...
            return (np.mean(
                values_drag_in_last_execute) + 0.159)  # TODO: the 0.159 value is a proxy value corresponding to the mean drag when no control; may depend on the geometry
