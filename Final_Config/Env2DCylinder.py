...
import tensorforce
from tqdm import tqdm  # Package to create progress bars
import numpy as np
...
# TODO: check that right types etc from tensorforce examples
# typically:
...
class RingBuffer():
    ...
    def get(self):
        ...
        idx = (self.index + np.arange(self.data.size)) % self.data.size  # Indices of 'data' from oldest to newest
...
class Env2DCylinder(Environment):
    ...
    def visual_inspection(self):  # Dynamic plotting AND command line information during runs
        ...
        if (not self.initialized_visualization and self.inspection_params["plot"] != False):  # Initialise visualization
        ...
        if ("plot" in self.inspection_params and self.inspection_params["plot"] != False):
            ...
            if self.solver_step % modulo_base == 0:   # Plots are updated every "plot" steps
    ...
    def sing_run_output(self):  # Output for single runs (testing of strategies)
        ...
        if (not os.path.exists("saved_models/" + name)):
            with open("saved_models/" + name, "w") as csv_file:
                ...
                spam_writer.writerow(
                    ["Name", "Step", "Drag", "Lift", "RecircArea"] + ["Jet" + str(v) for v in range(len(self.Qs))])
                spam_writer.writerow([self.simu_name,
                                      self.solver_step,
                                      self.history_parameters["drag"].get()[-1],
                                      self.history_parameters["lift"].get()[-1],
                                      self.history_parameters["recirc_area"].get()[-1]] + [str(v) for v in
                                                                                           self.Qs.tolist()])
        else:
            with open("saved_models/" + name, "a") as csv_file:
                spam_writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
    ...
    def output_data(self):
        ...
        if "dump" in self.inspection_params:
            ...
            if (self.last_episode_number != self.episode_number and "single_run" in self.inspection_params and
                    self.inspection_params["single_run"] == False):
                self.last_episode_number = self.episode_number
                avg_drag = np.average(self.episode_drags[len(self.episode_drags) // 2:]) # Avg for the 2nd half of each ep
                ...
                if (os.path.exists("saved_models/output.csv")):
                    if (not os.path.exists("best_model")):
                        ...
                        with open("saved_models/output.csv", 'r') as csvfile:
                            ...
                            for row in data:
                                lastrow = row
                            last_iter = lastrow[1]  # get avg drag of last episode

                        with open("best_model/output.csv", 'r') as csvfile:
                            data = csv.reader(csvfile, delimiter=';')
                            for row in data:
                                lastrow = row
                            best_iter = lastrow[1]  # get avg drag of best episode in best_model/
    ...
    def reset(self):
        ...
        # 20% probability of resetting everything. Why???
    ...
    def execute(self, actions=None):
        ...
        # to execute several numerical integration steps (no. of steps over which NN output (actions) is kept constant)
        for _ in range(self.number_steps_execution):
            ...
            if "smooth_control" in self.optimization_params:
                ...
                # printiv(actions)
                # printiv(self.Qs)
                # New control = c_s+1 = c_s + alpha * ( a - c_s )
    ...
    def compute_reward(self):
        ...
        if self.reward_function == 'plain_drag':  # a bit dangerous, may be injecting some momentum
            ...
            return (np.mean(
                values_drag_in_last_execute) + 0.159)  # TODO: the 0.159 value is a proxy value corresponding to the mean drag when no control; may depend on the geometry *dep
            ...
            return avg_drag + 0.159 - 0.2 * abs(avg_lift)   #*dep  # THE ONE USED IN PAPER I THINK
        elif self.reward_function == 'max_plain_drag':  # a bit dangerous, may be injecting some momentum
            values_drag_in_last_execute = self.history_parameters["drag"].get()[-self.number_steps_execution:]
            return - (np.mean(values_drag_in_last_execute) + 0.159)   #*dep
            ...
            avg_drag = np.mean(self.history_parameters["drag"].get()[-avg_length:])
            return avg_drag + 0.159 - 0.2 * avg_abs_lift   #*dep
