...
def convert(msh_file, h5_file):  # Takes path of .msh file and objective path of .h5 file
    '''Convert msh file to h5_file'''
    root, _ = os.path.splitext(msh_file)  # Turek_2d
    assert os.path.splitext(msh_file)[1] == '.msh'
    assert os.path.splitext(h5_file)[1] == '.h5'

    # Generate .xml mesh file
    xml_file = '.'.join([root, 'xml'])
    subprocess.call(['dolfin-convert %s %s' % (msh_file, xml_file)], shell=True)

    assert os.path.exists(xml_file)  # Sucess?

    # Generate .h5 mesh file
    mesh = Mesh(xml_file)   # Create FEniCS mesh file ???
    ...
    assert os.path.exists(h5_file)  # Sucess?
